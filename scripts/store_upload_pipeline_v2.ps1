param(
  ...
  [string]$OutRoot = "C:\dev\shadowprofile_store_upload"
)
...
$Out = Join-Path $OutRoot ("store_upload_v2_" + $Stamp)
...
$Browsers = @(
  ...
  @{ name = "brave"; zip = "shadowprofile_brave.zip"; note = "Brave users can install through Chrome Web Store or load unpacked." }
)

$shaPath = Join-Path $ReleaseRoot "sha256sums.txt"

if(-not (Test-Path $shaPath)){
  throw "MISSING_SHA256SUMS:$shaPath"
}
...
foreach($b in $Browsers){
  ...
  if(Test-Path (Join-Path $RepoRoot "store_assets")){
    Copy-Item (Join-Path $RepoRoot "store_assets") $Dir -Recurse -Force
  }

  $note = @"
ShadowProfile Store Upload Package v2
...
New in this package:
- Browser Baseline new-tab dashboard
- Local browser profile summary
- Local-only baseline JSON export
- Cross-browser PNG icon pack
...
Upload the zip in this folder to the browser store.
"@
...
Write-Host "SHADOWPROFILE_STORE_UPLOAD_V2_READY" -ForegroundColor Green
